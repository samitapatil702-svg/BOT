...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('zxcloud_bot.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('ZXCLOUDBot')
...
WATERMARK = "ZX CLOUD VPS Service"
WELCOME_MESSAGE = "Welcome To ZX CLOUD! Get Started With Us!"
MAX_VPS_PER_USER = int(os.getenv('MAX_VPS_PER_USER', '3'))
DEFAULT_OS_IMAGE = os.getenv('DEFAULT_OS_IMAGE', 'ubuntu:22.04')
DOCKER_NETWORK = os.getenv('DOCKER_NETWORK', 'bridge')
MAX_CONTAINERS = int(os.getenv('MAX_CONTAINERS', '100'))
DB_FILE = 'zxcloud.db'
BACKUP_FILE = 'zxcloud_backup.pkl'
...
async def build_custom_image(vps_id, username, root_password, user_password, base_image=DEFAULT_OS_IMAGE):
    ...
    try:
        ...
        # Build the image
        image_tag = f"zxcloud/{vps_id.lower()}:latest"
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    """Enhanced container setup with ZX CLOUD customization"""
    try:
        ...
        logger.info(f"Setting up container {container_id} for user {username}")
        ...
        if not use_custom_image:
            ...
            logger.info(f"Installing packages in {container_id}.")
                
            # Update package list
            logger.info(f"Running apt-get update in {container_id}.")
            ...
            logger.info(f"apt-get update successful in {container_id}.")
            ...
            logger.info(f"Installing packages in {container_id}: {packages}")
            ...
            logger.info(f"Packages installed successfully in {container_id}.")
        ...
        logger.info(f"Configuring SSH in {container_id}.")
        ...
        if not use_custom_image:
            ...
            for cmd in user_setup_commands:
                logger.info(f"Running user setup command in {container_id}: {cmd}")
                success, output = await run_docker_command(container_id, ["bash", "-c", cmd])
                if not success:
                    raise Exception(f"Failed to setup user: {output}")
            logger.info(f"User setup successful in {container_id}.")

        # Set UnixNodes customization
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("🎨 Setting up ZX CLOUD customization...", ephemeral=True)
        else:
            await status_msg.edit(content="🎨 Setting up ZX CLOUD customization...")
        logger.info(f"Customizing container {container_id}.")
        ...
        hostname_cmd = f"echo 'zxcloud-{vps_id}' > /etc/hostname && hostname zxcloud-{vps_id}"
        ...
        logger.info(f"Setting resource limits for {container_id}.")
        ...
        logger.info(f"Running security setup for {container_id}.")
        security_commands = [
            "ufw allow ssh",
            "ufw --force enable",
        ...
        for cmd in security_commands:
            logger.info(f"Running security command in {container_id}: {cmd}")
            success, output = await run_docker_command(container_id, ["bash", "-c", cmd])
            if not success:
                logger.warning(f"Security setup command failed: {cmd} - {output}")

        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("✅ ZX CLOUD VPS setup completed successfully!", ephemeral=True)
        else:
            await status_msg.edit(content="✅ ZX CLOUD VPS setup completed successfully!")
        logger.info(f"Container setup successful for {container_id}.")
...
bot = ZXCLOUDBot(command_prefix='/', intents=intents, help_command=None)
...
async def on_ready():
    ...
    try:
        await bot.change_presence(activity=discord.Game(name="ZX CLOUD"))
...
async def show_commands(ctx):
    ...
    try:
        embed = discord.Embed(title="🤖 ZX CLOUD VPS Bot Commands", color=discord.Color.blue())
...
@app_commands.describe(
    ...
    use_custom_image="Use custom ZX CLOUD image (recommended)"
)
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        status_msg = await ctx.send("🚀 Creating ZX CLOUD VPS instance... This may take a few minutes.")
        ...
        if use_custom_image:
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    volumes={
                        f'zxcloud-{vps_id}': {'bind': '/data', 'mode': 'rw'},
        ...
        await status_msg.edit(content="🔧 Container created. Setting up ZX CLOUD environment...")
        await asyncio.sleep(5)
        logger.info(f"Container {container.id} created for user {owner.id}. Starting setup...")
        ...
        if not setup_success:
            logger.error(f"Container setup failed for {container.id}.")
            raise Exception("Failed to setup container")
        logger.info(f"Container setup successful for {container.id}.")
        ...
        try:
            embed = discord.Embed(title="🎉 ZX CLOUD VPS Creation Successful", color=discord.Color.green())
            ...
            embed.add_field(name="ℹ️ Note", value="This is a ZX CLOUD VPS instance. You can install and configure additional packages as needed.", inline=False)
            
            await owner.send(embed=embed)
            await status_msg.edit(content=f"✅ ZX CLOUD VPS creation successful! VPS has been created for {owner.mention}. Check your DMs for connection details.")
...
async def list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your ZX CLOUD VPS Instances", color=discord.Color.blue())
...
async def admin_list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="All ZX CLOUD VPS Instances", color=discord.Color.blue())
        valid_vps_count = 0
...
async def delete_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await ctx.send(f"✅ ZX CLOUD VPS {vps_id} has been deleted successfully!")
    except Exception as e:
...
async def connect_vps(ctx, token: str):
    ...
    try:
        ...
        embed = discord.Embed(title="ZX CLOUD VPS Connection Details", color=discord.Color.blue())
...
4. You will be connected to your ZX CLOUD VPS
...
""".format(username=vps["username"]), inline=False)
        ...
        await ctx.send("✅ Connection details sent to your DMs! Use the Tmate command to connect to your ZX CLOUD VPS.", ephemeral=True)
...
async def admin_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="ZX CLOUD System Statistics", color=discord.Color.blue())
...
async def vps_usage(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your ZX CLOUD VPS Usage", color=discord.Color.blue())
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Emembed(title="ZX CLOUD Global Usage Statistics", color=discord.Color.blue())
